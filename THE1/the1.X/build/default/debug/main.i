...
main:
    ...
    setf TRISE ; PORTE is input, ((PORTE) and 0FFh), 0, a toggle C, ((PORTE) and 0FFh), 1, a toggle B

    movlw 00001111B
    movwf TRISA ; not used in this task
...
update_display:
    ...
    movlw 50 ; wreg = 250
    movwf counter1 ; counter1 = 250
    movlw 50 ; wreg = 250
    movwf counter2 ; counter2 = 250

    ; Toggle ((PORTD) and 0FFh), 0, a
    ; To toggle, we use xorwf with a mask that has a '1' for the bit we want to toggle
    movlw 0x01 ; Load the mask for ((PORTD) and 0FFh), 0, a
    xorwf LATD, F ; Toggle ((PORTD) and 0FFh), 0, a by performing an exclusive OR on the current value of LATD
    ...
    btfss progressB_enabled, 0 ; Skip next step if progress bar on PORTB is not enabled
    clrf LATB ; Clear the progress bar on PORTB

    update_display_end:
        return

; Subroutine to update progress bar on PORTB, start from 0th bit light everything until all of them are on then clear
...
increment_progressB:
    ...
    incf progressB_current, F ; Increment the progress counter

    ; Update the progress bar
    movf progressB_current, W ; Move the progress counter to W
...
check_Re0:
    ...
    ; Check for ((PORTE) and 0FFh), 0, a release
    btfsc currentRe0, 0 ; Check if ((PORTE) and 0FFh), 0, a is currently high, bit test file, skip if clear
    bsf previousRe0, 0 ; If it is high, set the corresponding bit in 'previous', bit set file
    btfss currentRe0, 0 ; Check if ((PORTE) and 0FFh), 0, a is currently low, bit test file, skip if set
    call re0_pressed ; If it is low, call the re0_pressed subroutine, relative call

    return

; Subroutine to handle ((PORTE) and 0FFh), 0, a button press
re0_pressed:
    ; Check if ((PORTE) and 0FFh), 0, a was previously high (1)
...
check_Re1:
    ...
    ; Check for ((PORTE) and 0FFh), 1, a release
    btfsc currentRe1, 1 ; Check if ((PORTE) and 0FFh), 1, a is currently high
    bsf previousRe1, 0 ; If it is high, set the corresponding bit in 'previousRe1'
    btfss currentRe1, 1 ; Check if ((PORTE) and 0FFh), 1, a is currently low
    call re1_pressed ; If it is low, call the re1_pressed subroutine

    return

; Subroutine to handle ((PORTE) and 0FFh), 1, a button press
re1_pressed:
    ; Check if ((PORTE) and 0FFh), 1, a was previously high (1)
